...
TAG_KEY_BACKUP_GENERATION = '7' #世代数
TAG_KEY_AUTO_BACKUP       = 'backup' #backup とるかどうかはこれで管理
TAG_VAL_AUTO_BACKUP       = 'True' #Trueのもののみbackup とる
...
def execute_ami_backup_task(ec2_client, ec2_resource):
    ...
    #バックアップした時の時間。amiの名前に使う
    exec_time = dt.now().strftime('%Y%m%d%H')
    ...
    for ec2_group in response['Reservations']:
        for instance_info in ec2_group['Instances']:            
            ...
            #TAG_KEY_BACKUP_GENERATION 以上の数があればami, snapshotを削除する
...
def create_buckup_image(ec2_client, ec2_resource, instance_info, exec_time):
    ...
    #"Nameタグの"Value＋時間"を AMI の Name の Value にして、AMIを作成する
...
#       ： ec2_resource
#       ： instance_info <dict>
# 機能　 ： 作られた日付で降順に並べて、保持世代よりも古いイメージを削除する
def delete_old_image(ec2_client, ec2_resource, instance_info):
    sorted_images = get_sorted_images(ec2_client, instance_info)

    #TAG_KEY_BACKUP_GENERATION で指定したよりも後の世代は削除する
    generation = int(TAG_KEY_BACKUP_GENERATION)
    cnt = 0
...
def construct_backup_tags(instance_info):
    ...
    tags = instance_info['Tags']
    for tag in tags:
        if (TAG_KEY_AUTO_BACKUP == tag['Key']):
            continue
...
#backup 可能かのテスト用に関数を叩いてみた
#event, context はawsの方で渡されるので、その中身を実行

#ec2_client   = boto3.client('ec2')
#ec2_resource = boto3.resource('ec2')
#execute_ami_backup_task(ec2_client, ec2_resource)